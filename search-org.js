function showSearchBox() {
    ...
    if (searchOptions === "mcqs" || searchOptions === "jobs") {
        ...
        searchText.textContent = `You are searching for ${searchOptions === "mcqs" ? "MCQs" : "Jobs"}.`;
    } else {
...
function performSearch() {
    ...
    // Show total count of MCQs or Jobs
    const totalCount = searchOptions === "mcqs" ? mcqsData.length : jobsData.length;
    const resultsInfo = document.createElement("p");
    resultsInfo.textContent = `You are searching for ${searchOptions === "mcqs" ? "MCQs" : "Jobs"}. Total ${searchOptions === "mcqs" ? "MCQs" : "Jobs"}: ${totalCount}`;
